...
<form class="am-form am-form-horizontal" id="editForm" name="editForm" method="post">
  <input type="hidden" name="id" value="${workMode.id}" />
  <div class="am-u-sm-4 no-padding">
    <div class="am-g am-margin-top">
      <input type="hidden" id="edit_workTime" name="workTimeIds"/>
      ...
      <div class="am-u-sm-8 no-padding-left no-padding-right">
        <select class="am-input-sm" id="edit_workTimes"  required data-am-selected="{maxHeight: 100,btnWidth:'117px'}">
  ...
  <div class="am-u-sm-4 no-padding">
    <div class="am-g am-margin-top">
      ...
      <div class="am-u-sm-8 no-padding-left no-padding-right">
        <input class="am-input-sm" type="text" value="${workMode.code}" placeholder="输入编号" required id="edit_code" name="code"  />
      </div>
    </div>
  </div>
  <div class="am-u-sm-4 no-padding">
    <div class="am-g am-margin-top">
      ...
      <div class="am-u-sm-8 no-padding-left">
        <input class="am-input-sm" type="text" value="${workMode.name}" placeholder="输入名称" required id="edit_name" name="name"  />
      </div>
    </div>
  </div>
  ...
  <div class="am-u-sm-12 no-padding">
    ...
    <div class="table-body" style=" height: 138px;">
      <table id="editWorkTimeDetail" class="am-table am-table-bordered am-table-striped am-table-hover no-margin-bottom" style="width:100%;">
        <tbody>
          <c:forEach items="${workModeTimes}" var="workModeTime" varStatus="status">
              <tr work-time-id="${workModeTime.id}">
                <td style="width: 8%; text-align: center;">${status.index+1}</td>
                <td style="width: 19%; text-align: center;">${workModeTime.code}</td>
                <td style="width: 19%; text-align: center;">${workModeTime.name}</td>
                <td style="width: 13%; text-align: center;">${workModeTime.beginTimeStr}</td>
                <td style="width: 13%; text-align: center;">${workModeTime.endTimeStr}</td>
                <td style="width: 13%; text-align: center;">${workModeTime.timeSub}</td>
                <td style="width: 15%; text-align: center;">
                  <a class="am-badge am-badge-danger am-radius am-text-lg"
                     onclick="removeProductDetail(this)" ><span class="am-icon-trash-o"></span>&nbsp;删除</a></td>
              </tr>
          </c:forEach>
        </tbody>
      </table>
...
<script type="text/javascript">
  $(function(){
          $("select").selected();
          $('#edit_workTimes').on('change',function(){
                  if($(this).val()=='-1'){
                          return false;
                          }
                  ...
                  $('#editWorkTimeDetail tbody').append('<tr work-time-id="'+$(this).val()+'">'+
                          '<td style="width: 8%;">'+($('#editWorkTimeDetail tr').length+1)+'</td>'+
